...
```{r}
filter(tree_dat, standID == 5, year == 2007) %>% arrange(desc(rad_ib_mm)) %>% slice_head(n = 3)
```

## Question 9: 
The smallest three trees measured in 2007 from stand 3 are trees 1514, 1103, 1508.
```{r}
filter(tree_dat, grepl("3$", stand), year == 2007) %>% select(treeID, stand, year, rad_ib_mm) %>% slice_min(order_by = rad_ib_mm, n = 3)
```
...
## Question 13:
```{r}
tree_dat <- tree_dat %>% rename(inc_unit_mm = inc_mm, rad_ib_unit_mm = rad_ib_mm) %>% glimpse()
```
## Question 14:
The mean BA is ~0.037 m^2
```{r}
mutate(tree_dat, rad_ib_cm = rad_ib_unit_mm / 10, DBH = 2 * rad_ib_cm, BA = 0.00007854 * DBH^2) %>% filter(species == "POTR", year == 2007) %>% summarise(mean_BA = mean(BA, na.rm = TRUE))
```
## Question 15:
...
```{r}
tree_dat <- tree_dat %>% mutate(rad_ib_cm = rad_ib_unit_mm / 10, DBH = 2 * rad_ib_cm)
```
```{r}
